...
| **Deterministic RequestIds** | RequestId = Gateway/Host Chain transaction hash or contract-assigned ID |
...
|-----------|------|----------|-------------|
| `requestId` | `bytes32` | Path | The request ID (hex-encoded) |
...
| `requestId` | `bytes32` | Echo of the request ID |
...
| `signature` | `bytes` | EIP-712 signature over the result |
...
| `signature` | `bytes` | EIP-712 signature over handles |
...
| `signature` | `bytes` | Signature over the ciphertext data |
| `signerAddress` | `address` | Coprocessor's signer address |
...
- `requestId` is unique per request (derived from on-chain counter or tx hash)
